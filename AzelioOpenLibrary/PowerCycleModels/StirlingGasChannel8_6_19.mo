...
equation
  ...
  der(E_wt) = Wt;
  sumFlows = Kscaling * Qh_port.Q_flow + Kscaling * Qc_port.Q_flow + Q_losses_port.Q_flow - Kscaling * Wt;
  Kscaling * Qh_port.Q_flow + Kscaling * Qc_port.Q_flow - Kscaling * Wt + Q_losses_port.Q_flow = 0;
  //if noEvent(nshaft > 0 and pme > 0) then
  if noEvent(nshaft > 0 and pme > 0) then
    Qh_port.Q_flow = Kscaling * (Qh_k + Qh_n * nshaft + Qh_pme * pme + Qh_Tc * Tc + Qh_HTCc * HTCc + Qh_Th * Th + Qh_n_pme * nshaft * pme + Qh_n_Tc * nshaft * Tc + Qh_n_HTCc * nshaft * HTCc + Qh_n_Th * nshaft * Th + Qh_n_2 * nshaft * nshaft + Qh_pme_Tc * pme * Tc + Qh_pme_HTCc * pme * HTCc + Qh_pme_Th * pme * Th + Qh_pme_2 * pme * pme + Qh_HTCc_2 * HTCc * HTCc + Qh_Th_2 * Th * Th + Qh_n_pme_Tc * nshaft * pme * Tc + Qh_n_pme_HTCc * nshaft * pme * HTCc + Qh_n_pme_Th * nshaft * pme * Th + Qh_n2_pme * nshaft * nshaft * pme + Qh_n_pme2 * nshaft * pme * pme + Qh_pme_HTCc2 * pme * HTCc * HTCc + Qh_pme2_HTCc * pme * pme * HTCc + Qh_n_HTCc2 * nshaft * HTCc * HTCc + Qh_n_Th2 * nshaft * Th * Th + Qh_pme_Th2 * pme * Th * Th);
    // Removed a minus in Qc_port.Q_flow. Flow is now negative, i.e. going from the engine to the cooler.
    Qc_port.Q_flow = Kscaling * (Qc_k + Qc_n * nshaft + Qc_pme * pme + Qc_Tc * Tc + Qc_HTCc * HTCc + Qc_Th * Th + Qc_n_pme * nshaft * pme + Qc_n_Tc * nshaft * Tc + Qc_n_HTCc * nshaft * HTCc + Qc_n_Th * nshaft * Th + Qc_n_2 * nshaft * nshaft + Qc_pme_Tc * pme * Tc + Qc_pme_HTCc * pme * HTCc + Qc_pme_Th * pme * Th + Qc_pme_2 * pme * pme + Qc_Tc_2 * Tc * Tc + Qc_HTCc_2 * HTCc * HTCc + Qc_Th_2 * Th * Th + Qc_n_pme_Tc * nshaft * pme * Tc + Qc_n_pme_HTCc * nshaft * pme * HTCc + Qc_n_pme_Th * nshaft * pme * Th + Qc_n2_pme * nshaft * nshaft * pme + Qc_n_pme2 * nshaft * pme * pme + Qc_pme_HTCc2 * pme * HTCc * HTCc + Qc_pme2_HTCc * pme * pme * HTCc + Qc_n_HTCc2 * nshaft * HTCc * HTCc + Qc_n_Th2 * nshaft * Th * Th + Qc_pme_Th2 * pme * Th * Th);
    Wt = Kscaling * (Wt_k + Wt_n * nshaft + Wt_pme * pme + Wt_Tc * Tc + Wt_HTCc * HTCc + Wt_Th * Th + Wt_n_pme * nshaft * pme + Wt_n_Tc * nshaft * Tc + Wt_n_HTCc * nshaft * HTCc + Wt_n_Th * nshaft * Th + Wt_n_2 * nshaft * nshaft + Wt_pme_Tc * pme * Tc + Wt_pme_HTCc * pme * HTCc + Wt_pme_Th * pme * Th + Wt_pme_2 * pme * pme + Wt_Tc_2 * Tc * Tc + Wt_HTCc_2 * HTCc * HTCc + Wt_Th_2 * Th * Th + Wt_n_pme_Tc * nshaft * pme * Tc + Wt_n_pme_HTCc * nshaft * pme * HTCc + Wt_n_pme_Th * nshaft * pme * Th + Wt_n2_pme * nshaft * nshaft * pme + Wt_n_pme2 * nshaft * pme * pme + Wt_pme_HTCc2 * pme * HTCc * HTCc + Wt_pme2_HTCc * pme * pme * HTCc + Wt_n_HTCc2 * nshaft * HTCc * HTCc + Wt_n_Th2 * nshaft * Th * Th + Wt_pme_Th2 * pme * Th * Th);
  else
    Qh_port.Q_flow = 0;
    Qc_port.Q_flow = 0;
    Wt = 0;
